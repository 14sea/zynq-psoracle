...
def manifest_v07(with_import: bool = True, n_rule: str = "policy_matched_wall", **over) -> dict:
    m = copy.deepcopy(L6M)
...
class ExplicitImport(unittest.TestCase):
    ...
    def test_with_the_declaration_the_soak_plans_and_records_the_import(self):
        ...
        self.assertEqual(p["n"], 12511)
        self.assertEqual(sorted(p["inputs"]["calibrations_imported"]), ["C1", "C2"])
        self.assertEqual(p["inputs"]["calibrations_imported"]["C1"]["from_prereg_version"], "v0.6")
        self.assertEqual(p["inputs"]["calibrations_imported"]["C1"]["from_prereg_sha256"], L6M["prereg"]["sha256"])
    ...
    def test_an_import_naming_the_wrong_prereg_hash_is_refused(self):
        ...
        with self.assertRaises(ValueError) as cm:
            self.plan(m)
        self.assertIn("the import names prereg", str(cm.exception))
