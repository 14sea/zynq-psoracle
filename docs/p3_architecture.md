...
### 3c. Key custody and provisioning — an architecture requirement, not a note (v0.3, option A)

- **Principals.** `gate-signer` and `runner` are **different OS users** on the host. The
  runner never holds `K`, never reads a file that contains it, and has no access to the
  JTAG pod. The gate signer holds `K` (`0400`, owned by `gate-signer`), owns the pod, and
  signs only after its own whitelist verdict.
- **The key is not in the bitstream.** The carrier has a **write-only, write-once key
  register** (`0x2160‥216C`, then CTRL bit 8 `key_commit`; `STATUS` bit 11 `key_loaded`).
  After every configuration the register is empty and every ARM is refused with
  `F_ARM_NOKEY` until the signer provisions `K` **over the DAP mem-AP (JTAG)** — a path the
  console, and so the runner, never has. Reads of the register are SLVERR; a second write
  is SLVERR; only a reconfiguration clears `key_loaded`. `K` lives in flip-flops, not in
  configuration frames, so PCAP/JTAG frame readback cannot see it (GCAPTURE is forbidden by
  the line's rules). **The bitstream is therefore public and reproducible** — one build,
  committed, no keyed variant.
- **Provisioning is a board action** with its own ruling text (`provisioning P3-K`); the
  signer's tool (`host/provision_key_jtag.py`) prepares the openocd script host-side and
  executes it only with that ruling. The runner asks for provisioning through the signer's
  request interface and then *observes* `key_loaded` before any ARM; it records the
  observation (`arm_record.key_loaded_observed`).
- **Rotation.** A new key = a new provisioning (same bitstream); `key_id = sha256(K)` is
  recorded per session in `arm_record.signer.key_id`, not in the manifest.
- **Failure behaviour.** Key file unreadable or absent → the signer refuses (no ARM). Not
  provisioned → `F_ARM_NOKEY`, nonce consumed, sticky. Wrong key provisioned → `F_ARM_AUTH`.
  Both are L3 pre-positive negative controls, on the fake and on the board.
- **Non-readability checks.** L1 exit: no AXI address returns the key words; the MAC core
  sits in the logic pblock, outside the readable target columns; bench: read → SLVERR,
  rewrite → SLVERR. Host: a test run *as the runner user* must fail to open the key file
  and must not be in the pod's group.
...
- **F2 — PL custody is not enforced by this architecture.** Whoever holds the console can
  `fpga loadb` another bitstream, including one whose key they know, and the signer would
  then be provisioning a PL it did not build. The session pins the setup-load sha and the
  run log records it, but that is host evidence, not a hardware boundary. Option A closes
  key custody (F1); it does **not** close F2, which stays inside the threat model's
  exclusion of a compromised host and is stated here so no reader takes A for more than it is.
