...
|---|---|---|---|---|
| **L0** host-only architecture (this document, contracts, decisions) | cross-reviewed; contracts have validators + fixtures; every imported artifact has a manifest row with sha256 and source commit | reviewed | a reviewer finds the predicate in §3 can be satisfied without an instrument observing bytes | a reviewer shows §3 is a bypass of the carrier's interlock rather than a re-establishment |
...
| **L2 = P2b** counter-class non-perturbation (first board stage, own ruling) | P2's protocol on the P3 carrier with the invariant replaced by *the heartbeat advances within a pinned envelope* during ten PCAP reads and one write, against a matched no-read control | envelope held, control stable | control unstable (observable non-discriminating) | attributable violation: PCAP activity stalls or perturbs the computing design |
