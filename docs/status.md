...
| L5 the loop | **HOLD — session 1 ran on hardware and STOPPED at the ARM.** First execution of the firmware ever (ruling `P3-L5 2026-09-01-01`, evidence `evidence/l5_17A6_2026-09-01-01/`). Everything before the ARM worked on silicon: identity and `IDENT`, the notary round trip, staging, link 2, DMA, link 3, and an **8-chunk audit whose raw words independently recompute both link-2 and link-3 hashes to the signed commit**. Preflight `CPU_CLK_CTRL = 0x1f000200` → CPU_6x4x = 666.67 MHz, discharging the assumption (the 6:2:1 bit at SLCR 0x1C4 is still unread). The stop: after the payload, tag and `ARM_STROBE` writes the **nonce did not step**, so the PL never saw the ARM. Ruled out: the app's own write allowlist, a cacheable PL window (the BSP maps it Strongly Ordered), and wrong offsets/strobe (identical to the L3 sequence that worked). **Root cause not determined.** Zero disruptions, zero CRC drops; base restored; no kill criterion triggered. Both rulings consumed — a retry needs a power cycle and new ones. Pinned image `d3828a8c…`. | HOLD per `docs/l5_prereg.md` §5 (a STOPPED epoch is a HOLD, not a KILL) | `docs/l5_session1_findings.md`, `evidence/l5_17A6_2026-09-01-01/`, `docs/l5_post_build_package.md` |
