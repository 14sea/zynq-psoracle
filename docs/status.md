...
| L5 the loop | **NOT READY FOR BOARD.** Rounds 3 and 4 are closed (the wire defect, then the unimplementable audit condition); the round-5 **post-build evidence package** (`docs/l5_post_build_package.md`, baseline `d3828a8c…`) is submitted and awaits review — it gates push, the rulings and the first N = 8 session. History: the round-2 package passed and authorised push + rulings + board, but its last step had no executable, and looking for that found the C application's framed output had never been checked against the host validator (no `IDENT`, no heartbeat, no audit, flat `loop_record`s, a closing control that is not a `LOOP_OUTCOME`) — the rehearsal had only ever exercised the Python reference loop. Round 3 then found "audit every candidate" unimplementable: a gate refusal stages nothing, so no raw words exist. Now: a link-2 refusal serves its staged words before its record (`span` distinguishes a short audit from a full one), gate refusals are exempt **and recorded as exempt**, and the policy is machine-checked by `validators.records.check_audit_policy`, which the runner calls — a `PASS` cannot be reported if a self-reporting candidate went unaudited. Pinned image **`d3828a8c…`**; `7540239f…` and `b279459c…` withdrawn (`docs/l5_wire_findings.md` §7). **The firmware has still never run on hardware — every green result is host-side.** Nothing pushed, no ruling, no board contact. | rounds 2–4 host-only, owner's authorisation; no rung adjudication claimed | `docs/l5_post_build_package.md` (entry point), `docs/l5_wire_findings.md`, `docs/l5_prereg.md`, `manifests/l5_manifest.json`, `host/l5_runner.py`, `evidence/l5_build/`, `docs/l5_review_package.md` |
