...
| L5 the loop | **HOLD. Session 2 (the diagnostic run) CRASHED on a defect I introduced, so session 1's question is still open.** Session 1 (`evidence/l5_17A6_2026-09-01-01/`, image `d3828a8c…`) was the firmware's first execution: identity, `IDENT`, notary, staging, link 2, DMA, link 3 and an 8-chunk audit all held on silicon — the audit's raw words independently recompute both link hashes to the signed commit — and the preflight gave `CPU_CLK_CTRL = 0x1f000200` → CPU_6x4x 666.67 MHz (`CLK_621_TRUE` at SLCR `0x1C4` still unread). It stopped because the nonce did not step: the PL never consumed the ARM; **root cause undetermined**. The instrumentation batch then added `STOP_ARM` and full ARM observations — but one of those observations, a read of `CTRL`, is an **SLVERR on this carrier** (`rtl/p3_axil.v`: `0x2000 CTRL` is **write-only**), so session 2 (`evidence/l5_17A6_2026-09-01-02/`, image `8390c463…`) took a data abort right after the audit and died silently: `CRASHED`, zero records, **required output not produced** — an instrumentation failure by `docs/l5_diag_spec.md` §2, not a hardware finding. I widened `axi_readable()` on a false premise (the `md.l CTRL` in `l3_runner.py` is DEVCFG's `0xF8007000`, not the PL's `0x2000`). **`ctrl_before`/`ctrl_after` are unobservable from the PS on this carrier; §4 of the session-2 findings puts the options to the owner.** Not pushed, both `-02` rulings consumed, no fix applied. | HOLD per `docs/l5_prereg.md` §5 | `docs/l5_session2_findings.md`, `docs/l5_session1_findings.md`, `docs/l5_diag_spec.md` |
