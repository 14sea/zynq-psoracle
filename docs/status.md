...
| L6 calibration + soak | **DRAFT preregistration — not authorised, no image, no ruling, board untouched.** Written 2026-09-01 on `zynq-fabricmap`'s ruling (Claim B RESUMPTION-ELIGIBLE, still PAUSED until this passes and the two-operator image completes P3 compatibility review). Requires host-only instrument changes first (frame timestamps — session 4 has none, so the per-candidate rate is unmeasured; a rate report; the sampled audit policy; an arm-aware validator; ruling text `whole-of-probe P3-L6`), then the two-operator image, then sessions C1/C2 (calibration, N = 64) and S (2 h soak). D-s1..D-s4 ruled 2026-09-01 (watchdog ON 30 s flag-gated; sampled audit with the §3a timing rule — non-`SCORED` self-reports auto-audited by the firmware before the record, since `AUDITREQ` precedes staging; 2 h with N from the two calibration records' hashes; CRC budget by expected frame count, closed formula). Implementation of §4 and §2 not yet authorised. | — | `docs/l6_soak_prereg.md` |
