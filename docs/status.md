...
| L5 the loop | **HOLD — session 1 ran and STOPPED at the ARM; the authorised instrumentation batch is done and a new package awaits review. No retry has been made.** Session 1 (`evidence/l5_17A6_2026-09-01-01/`, image `d3828a8c…`) was the firmware's first execution: identity, `IDENT`, the notary round trip, staging, link 2, DMA, link 3 and an 8-chunk audit all held on silicon — **the audit's raw words independently recompute both link hashes to the signed commit** — and the preflight gave `CPU_CLK_CTRL = 0x1f000200` → CPU_6x4x = 666.67 MHz (the `CLK_621_TRUE` bit at SLCR `0x1C4` is still unread). The stop: the nonce did not step, so the PL never consumed the ARM; **root cause undetermined**, with the app's write allowlist, a cacheable PL window and wrong offsets/strobe all ruled out. The batch since: the ARM path preserves `STATUS`/`FAULT`/`CTRL`/write-count/both nonces on every path, `STOP_ARM` records a non-consumed ARM (instrumentation only — it consumes no nonce in the chain, and a stepped nonce rejects it), and only a `COMPLETED` epoch can be a `PASS`. Pinned image now **`8390c463…`**. **The result was not explained away and no rule was relaxed.** **Next: a DIAGNOSTIC session, specified in advance at `docs/l5_diag_spec.md` — not a retry; a PASS is not its objective and would itself only mean the failure is intermittent.** Not pushed, no new ruling, board untouched since session 1. | HOLD per `docs/l5_prereg.md` §5 | `docs/l5_session1_findings.md`, `docs/l5_post_build_package.md`, `evidence/l5_17A6_2026-09-01-01/` |
