...
| L5 the loop | **host-only work COMPLETE, awaiting the D5 batch review** (`docs/l5_review_package.md`): D1 reviewed ACCEPTED WITH Q7 CONDITION (met — corpus `N = 256`); firmware written, its pure half host-compiled and **bit-exact against Python over all 256 corpus entries**; source audit green; manifest, preregistration (N = 8, every candidate audited) and review package written. **`firmware/p3_app.c` has never been compiled** (no ARM toolchain; the toolchain is decision D-a of the package). **No L5 build, no board contact, no ruling** | review #1 HOLD → review #2 ACCEPTED WITH Q7 CONDITION (`docs/d1_review_result.md`); the batch review is pending | `docs/l5_review_package.md`, `docs/l5_prereg.md`, `manifests/l5_manifest.json`, `docs/l5_design.md`, `docs/d1_standalone_spec.md`, `decisions.md` D1/D5 |
