...
| L6 calibration + soak | **Prereg v0.2 reviewed by the owner (2026-09-01); §4 host-only instrument batch delivered, re-review HOLD (four semantic defects: heartbeat rule over any frame, STOP_AXI exempt by name, closing transition in the rate, mutation_bits treated as instrument-only) — corrected the same day with two-way discrimination tests; second re-review HOLD on one blocker (session-wide HB count let a log whose heartbeats stopped after the second pass) → per-record HB completeness gate (exactly 16 per SCORED record, shared by C1/C2/S) added; awaiting the final short re-review — no image, no ruling, board untouched.** v0.2 pushed (`04d09ea`); D-s1..D-s4 ruled and §3a fixed (watchdog ON 30 s flag-gated; sampled audit with non-`SCORED` self-reports auto-audited by the firmware before the record; 2 h with N from the two calibration records' hashes; CRC budget by expected frame count, closed formula). §4 delivered host-only (`docs/l6_instrument.md`): per-frame timestamps + `console.ts.log` (`host/l6_timing.py`, attribution proven on session 4's real frame order), `host/l6_rate.py` (refuses session 4's untimed log), the sampled policy with §3a's two negatives on session 3's real STOP_ARM words, the arm-aware validator, `whole-of-probe P3-L6` in `host/l6_runner.py` (fail-closed: refuses on the draft manifest's null pins), N/timeout from calibration-record hashes, expected frames + CRC budget before the session, the A,B,B,A schedule and the two operators' Python reference with a 256-pair twin corpus. Corrections in `docs/l6_instrument.md` §7. Next: owner's short re-review → push → authorises §2 (firmware/image) → byte-identical rebuild → P3 compatibility review → freeze → C1/C2/S rulings. | — | `docs/l6_soak_prereg.md`, `docs/l6_instrument.md`, `manifests/l6_manifest.json` (DRAFT) |
