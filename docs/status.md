...
| L5 the loop | **NOT READY FOR BOARD — the image was rebuilt after a wire-protocol defect; a new post-build package awaits review.** The post-build re-review passed and authorised push + rulings + board, but the authorised sequence's last step had no executable, and looking for it found that the C application's framed output had never been checked against the host validator: no `IDENT` (which the validator requires), no heartbeat, no audit, `loop_record`s with no `seq`/`verified`/`evidence`, and the closing control emitted as a non-`LOOP_OUTCOME`. The rehearsal had only ever exercised the Python reference loop. **Image `7540239f…` is WITHDRAWN**; the pinned image is `b279459c…`. Fixed host-only: `firmware/p3_wire.{c,h}` + `tests/test_firmware_wire_contract.py` (the C bytes judged by the real validator), `p3_app.c` sends IDENT/HB/AUDIT/CLOSE and correctly shaped records, `host/l5_runner.py` exists. Full account: `docs/l5_wire_findings.md`. **The firmware has still never run on hardware.** Nothing pushed, no ruling, no board contact. (Historical, superseded: the earlier BSP-provenance HOLD and its fix, `docs/l5_review_result.md`.) Review 2026-08-31 (`docs/l5_review_result.md`): one provenance blocker — the embeddedsw BSP sources build.sh compiles were unpinned, so `app_image_sha256` was reproducible only against this host's tree. Fixed by pinning them (not vendoring): `manifests/l5_bsp_inputs.json` (65 files = every source + header closure, from `gcc -M`, via `host/gen_bsp_input_manifest.py`), a re-hashing guard test, a post-build evidence bundle (`evidence/l5_build/`, `host/gen_build_evidence.py`), and prereg §7 marking the additions as strengthening. Image rebuilt byte-identical (`7540239f…`); no firmware changed; not pushed. Toolchain pinned (xPack arm-none-eabi-gcc 14.2.1-1.1, sha256 verified); a hand-assembled cortex-a9 standalone BSP compiles `p3_app.c`+`p3_derive.c`+`p3_search.c` **`-Wall -Wextra` clean, no undefined symbols** into a linked image (≈1.19 MiB at 0x0200_0000, reset vector → `_boot`, clear of the buffers); linker-map validated; `p3_derive.c` unchanged, so the 256-entry corpus evidence carries over. **D-c ruled: watchdog OFF for session 1** (option 2) — identity `flags.bit1 = 0`, `watchdog_load_value` *not used*, host recovery fixed in prereg §4, a new audit test checks the WDT is touched only under that flag. No firmware changed, so **`app_image_sha256` is final** (`7540239f…`, byte-identical rebuild after the ruling). **Preflight before board:** one `md.l 0xF8000120 1` — CPU_6x4x/PERIPHCLK are assumed 6:2:1 until then. **Board, rulings and any long run still paused; nothing pushed.** | D5 batch review = PASS host-only; D-c = option 2 (owner, 2026-08-31). Next owner items: the post-build package, then push / `P3-L5` + `P3-K` rulings / the first N = 8 session | `docs/l5_findings.md`, `docs/l5_prereg.md`, `manifests/l5_manifest.json`, `firmware/bsp/`, `docs/decisions.md` addendum 2026-08-31, `docs/l5_review_package.md` |
