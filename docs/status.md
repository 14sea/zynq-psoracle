...
| L5 the loop | **D5 batch review PASSED (host-only); build authorised and DONE except one decision.** Toolchain pinned (xPack arm-none-eabi-gcc 14.2.1, sha256 verified); a hand-assembled cortex-a9 standalone BSP compiles `p3_app.c`+`p3_derive.c`+`p3_search.c` **`-Wall -Wextra` clean, no undefined symbols**, into `firmware/bsp/out/p3_app.elf` (image ≈1.19 MiB at 0x0200_0000, reset vector → `_boot`); linker-map validated; `p3_derive.c` unchanged so §5.2 is moot. **ONE build finding blocks manifest finalisation:** D-c's 30 s watchdog is unreachable at prescaler 0 (32-bit @ 333 MHz → 12.88 s max) and needs a `p3_app.c` prescaler write — owner's decision (`docs/l5_findings.md` §4). `app_image_sha256` + `watchdog_load_value` held null until then. **Board, ruling and any long run still paused.** | D5 review = PASS host-only (owner, 2026-08-31); the watchdog decision + the post-build package are the owner's next items | `docs/l5_findings.md`, `firmware/bsp/`, `manifests/l5_manifest.json`, `docs/l5_review_package.md`, `docs/l5_prereg.md`, `docs/l5_design.md` |
