...
| **D-b1** the malformed-line policy (PROPOSED) | §2.6q: a malformed `P3L5` line belonging to no open transaction is ledgered once as a `BAD_FRAME`, not acknowledged, not signed, advancing no seq, not refreshing liveness, and is NOT handed to the collector; the board's bounded resend recovers the frame and the collector's silence rule still ends an epoch that goes quiet. **Bounded**: `bad_frame_budget` = D-s4's formula (`ceil(4 × expected_protocol_frames / 1000)`, the same number as the CRC budget) over the ledger's `bad_frames`; the first past it ends the epoch `PROTOCOL_BAD_FRAME_BUDGET`, and §6.14 names the total in the adjudication even when it did not. Proven host-only on S #2's recorded bytes (the host survives the line) plus a modelled byte-identical `REC 145` resend (accepted once, `RECACK`, normal progress) with the negatives — no resend, wrong resend, conflicting resend, the malformed line repeated, budget exhaustion — and a 12-seed session soak (`docs/l6_s2_host_batch_package.md`). |
...
23. **The firmware batch** (opened by the owner 2026-09-02, delivered 2026-09-03):
    ...
    promoted to `pinned_at_build` (protocol rel-v4, `board_ready: true`) at this freeze.
...
14. **bad frames bounded (D-b1)**: the ledger's `bad_frames` ≤ `bad_frame_budget`
   (D-s4's formula, the same number as the CRC budget). The console ends the epoch
   `PROTOCOL_BAD_FRAME_BUDGET` at the first one past it; this condition names the total in
