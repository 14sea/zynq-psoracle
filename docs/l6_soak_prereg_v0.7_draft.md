...
> owner's freeze — it stands on its own and does not refer the reader to v0.6 for any
> rule.** v0.6 (`docs/l6_soak_prereg.md` at the freezing commit, sha `bfd69d10…`, frozen
> 2026-09-03) stays frozen and in force until then. Under v0.6 the rel-v4 image
...
any text that does not hash to the pin. **That is not yet the case for v0.7**: v0.6 stays
the frozen text until the owner reviews this batch and freezes this one. v0.6, v0.4, v0.3
and v0.2 are superseded, not erased — their hashes and texts live in git history and in
...
7. Review by the owner against this list, item by item, before any L6 ruling; the image
   ...
   *(Items 1–6f: reviewed and pinned 2026-09-02 for v0.4. Items 6g–6h: the transport
   ...
   `pinned_at_build` and this document is frozen, 2026-09-03. Items 6q–6r: the host batch
   after S #2, delivered 2026-09-03 (`docs/l6_s2_host_batch_package.md`), host-only, no
   image change — under review; this text is frozen only if that review passes.)*

## 3. Decisions — RULED by the owner (D-s1..D-s4 2026-09-01; D-r1..D-r5, D-t1..D-t3, D-p1 2026-09-02) and PROPOSED for v0.7 (D-b1, D-h1, D-n1, D-i1 — the owner rules them at this freeze)
...
3a. **The host batch after S #2** (`docs/l6_s2_host_batch_package.md`, delivered
   2026-09-03, host-only, no image change): §2.6q and the v0.7 rules as code behind the
   prereg version, the replay of S #2's recorded bytes, the modelled resend and its
   negatives, the session soak, the N/T comparison. Then the owner's review, the rulings
   D-b1 / D-h1 / D-n1 / D-i1, and the freeze of THIS text as v0.7 (sha into the manifest,
   v0.6 superseded in history).
