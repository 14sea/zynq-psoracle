...
| L3 one gated candidate end-to-end | **session #1 (unprovisioned) STOP LINK3_MISMATCH** — three envelopes written, `0x00400A20` read back BLANK although L2 read the same envelope-0 write bit-exact; no ARM; diagnostic session proposed (`docs/l3_findings.md`). Previously: **ready for the board** (runbook `docs/l3_l4_runbook.md`; real-signer rehearsal on the fake passes); **host tooling written** (`host/l3_runner.py`, link-1 gate, host oracle pinned to fabricmap's silicon scores, out-of-process signer, on-board negative controls; `docs/l3_design.md`); **no ruling, not authorised** |
