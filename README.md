...
| L2 = P2b counter-class non-perturbation | **two board runs on 17A6, both host-instrument outcomes** (`docs/l2_findings.md`): #1 killed by a shell timeout after 10/10 reads PASS; #2 board sequence complete (10/10 reads, write, readback bit-exact, state held on all samples, 12/14 heartbeat intervals in envelope) but the adjudicator could not decide intervals longer than the 85.9 s counter wrap → **spec v1.1 sub-samples** (`docs/l2_spec.md`); **run #3 needs the owner's review of v1.1 and a new ruling** |
